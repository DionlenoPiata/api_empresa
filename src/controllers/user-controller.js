...
exports.post = async (req, res, next) => {
    ...
    try {
        ...
        if (userExist) {
            res.status(200).send({ success: false, message: 'Já existe um usuário com esse email!' });
            return;
        }
        await dao.create({
            name: data.name,
        ...
        res.status(201).send({ success: true, message: 'Cadastro realizado com sucesso!' });

    } catch (e) {
        res.status(500).send({ success: false, message: 'Falha ao processar a requisição!' });
    }
}
...
exports.authenticate = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) {
            res.status(404).send({ success: false, message: 'Usuário ou senha inválidos!' });
            return;
        }
